...
namespace CPMBusinessLayer
{
	...
	// 2019-03-16 : DvdB : Renamed Class
	public class ProductionBusinessLayer : IDisposable
	{
		...
		public bool LoadAllLevel(string Thelevel)
		{
			...
			try
			{
				...
				Bus_Logic.SqlStatement = "Select HierarchicalID,Description from hierarch \r\n " +
						...
						"       HierarchicalID <= " + TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID + " +2 " +
						" union all \r\n " +
						" select HierarchicalID = " + TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID + " +3 ," +
							" 'Crew' Description " +
						 " union all \r\n " +
						" select HierarchicalID = " + TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID + " +4 ," +
		...
		public bool LoadLevel(string Thelevel)
		{
			...
			try
			{
				...
				Bus_Logic.SqlStatement = "Select HierarchicalID,Description from hierarch where hierarchicalType = 'Pro' and HierarchicalID > " + Thelevel + " and HierarchicalID <= " + TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID + " + 1";
		...
		public bool get_SectionsReportsNotSaved(string prodmonth, string activity)
		{
			...
			try
			{
				...
				sb.AppendLine("dbo.fn_FileExists('" + TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).PlanProtSaveDir + "' + [FileName]) FileExsits ");
		...
		public bool GetPlan()
		{
			...
			try
			{
				...
				sb.Clear();
				sb.AppendLine("SELECT 1 Code, 'Monthly Dynamic Plan' [Descd] ");
				sb.AppendLine("UNION ");
				sb.AppendLine("SELECT 2 Code, 'Monthly Lock Plan' [Descd] ");
				sb.AppendLine("UNION ");
				sb.AppendLine("SELECT 3 Code, 'Cycle Dynamic Plan' [Descd] ");
				sb.AppendLine("UNION ");
				sb.AppendLine("SELECT 4 Code, 'Cycle Lock Plan' [Descd] ");
				sb.AppendLine(" ");
				Bus_Logic.SqlStatement = sb.ToString();
				Bus_Logic.ExecuteInstruction();
		...
		public bool GetPlanSectionsAndNameADO(String Prodmonth)
		{
			...
			try
			{
				...
				for (int i = 1; i < 6; i++)
				{
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID)
					{
					...
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID - 1)
					{
					...
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID - 2)
					{
					...
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID - 3)
					{
					...
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID - 4)
					{
					...
					if (i == TMinewasteGlobal.getSystemSettingsProductionInfo(_TUserCurrentInfo.Connection).MOHierarchicalID - 5)
					{
		...
		/// <summary>
		/// Get the list of workplaces not planned 
		/// </summary>
		/// <param name="ProdMonth"></param>
		/// <param name="Activity"></param>
		/// <returns>DataTable with result</returns>
		/// 2019-09-16 : DvdB : Added this to replace duplicated code in revised planning
		public DataTable GetUnplannedWorkplaceList(string ProdMonth, string Activity)
		{
			MWDataManager.clsDataAccess WorkplaceData = new MWDataManager.clsDataAccess();
			WorkplaceData.ConnectionString = TConnections.GetConnectionString(_systemDBTag, _TUserCurrentInfo.Connection);
			WorkplaceData.queryExecutionType = MWDataManager.ExecutionType.GeneralSQLStatement;
			WorkplaceData.queryReturnType = MWDataManager.ReturnType.DataTable;

			StringBuilder sb = new StringBuilder();
			sb.AppendLine("Select distinct w.WORKPLACEID, w.DESCRIPTION from WORKPLACE w");
			sb.AppendLine("Where (w.Activity in ('" + Activity + "')) ");
			sb.AppendLine("AND ((w.Inactive <> 'Y') OR (w.Inactive IS NULL)) ");
			sb.AppendLine("AND w.Workplaceid not in ( Select Workplaceid from (");
			sb.AppendLine("Select pre.Workplaceid from PLANMONTH pre where (ISNULL(isStopped,'')='' or ISNULL(isStopped,'')='0' or ISNULL(isStopped,'')='N')  and  Prodmonth = " + ProdMonth + " and ISNULL(AutoUnPlan,'')='' ");
			sb.AppendLine("union ");
			sb.AppendLine("select [WorkplaceID] from [dbo].[PREPLANNING_CHANGEREQUEST] where Prodmonth = " + ProdMonth + " and [ChangeID]in(2,5)) a where a.workplaceid is not null)\r ");
			sb.AppendLine(" union ");
			sb.AppendLine(" select wp.WORKPLACEID, wp.DESCRIPTION from PREPLANNING_CHANGEREQUEST ppcr inner join PREPLANNING_CHANGEREQUEST_APPROVAL ppcra on");
			sb.AppendLine("  ppcra.changerequestid=ppcr.changerequestid inner join workplace wp on ");
			sb.AppendLine("  wp.workplaceid=ppcr.workplaceid where ");
			sb.AppendLine(" ppcra.declined=1 and ppcra.approved=0 and ");
			sb.AppendLine("  ppcr.prodmonth=" + ProdMonth);
			sb.AppendLine(" AND ppcr.Workplaceid not in (");
			sb.AppendLine(" Select pre.Workplaceid from PLANMONTH pre where ISNULL(isStopped,'')='' and  Prodmonth = " + ProdMonth + " and ISNULL(AutoUnPlan,'')='')");

			WorkplaceData.SqlStatement = sb.ToString();
			WorkplaceData.ExecuteInstruction();

			return WorkplaceData.ResultsDataTable;
		}
	}
}
