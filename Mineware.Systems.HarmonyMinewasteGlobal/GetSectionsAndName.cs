...
namespace Mineware.Systems.MinewasteGlobal
{
    public class GetSectionsAndName
    {
        ...
        public MWDataManager.clsDataAccess theSectionsAndName(string ProdMonth, string ForWhom, int WhichLevel)
        {
            ...
            if (WhichLevel == TMinewasteGlobal.getSystemSettingsProductionInfo(UserCurrentInfo.Connection).MOHierarchicalID - 3)
            {
            ...
            if (WhichLevel == TMinewasteGlobal.getSystemSettingsProductionInfo(UserCurrentInfo.Connection).MOHierarchicalID - 2)
            {
            ...
            if (WhichLevel == TMinewasteGlobal.getSystemSettingsProductionInfo(UserCurrentInfo.Connection).MOHierarchicalID - 1)
            {
            ...
            if (WhichLevel == TMinewasteGlobal.getSystemSettingsProductionInfo(UserCurrentInfo.Connection).MOHierarchicalID)
            {
    ...
    public class getSectionProdMonth
    {
        public TUserCurrentInfo UserCurrentInfo;
        public string theSystemDBTag;
        public DateTime GetSectionCurrentPeriod(string Section)
        {
            MWDataManager.clsDataAccess _dbMan = new MWDataManager.clsDataAccess();
            _dbMan.ConnectionString = TConnections.GetConnectionString(theSystemDBTag, UserCurrentInfo.Connection);
            //_dbMan.ConnectionString = ConfigurationManager.AppSettings["SQLConnectionStr"];

            _dbMan.SqlStatement = "sp_Report_GetSectionCurrentPeriod @Section='" + Section + "' ";
            _dbMan.queryExecutionType = MWDataManager.ExecutionType.GeneralSQLStatement;
            _dbMan.queryReturnType = MWDataManager.ReturnType.DataTable;
            _dbMan.ExecuteInstruction();
            DataTable SecCal = _dbMan.ResultsDataTable;


            return Convert.ToDateTime(SecCal.Rows[0]["Prodmonth"].ToString());
        }
    }
}
